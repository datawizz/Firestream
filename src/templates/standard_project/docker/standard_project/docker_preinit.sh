...
# Define file paths
BASE_FILE="${PROJECT_ROOT}/docker/standard_project/docker-compose.yml"
TEMP_COMPOSE_FILE="${PROJECT_ROOT}/docker/standard_project/docker-compose.temp.yml"
...
if [ "$HOST_GPU_VENDOR" = "NVIDIA" ]; then
    OVERRIDE_FILE="${PROJECT_ROOT}/docker/standard_project/docker-compose.gpu_nvidia.yml"
    ...
    if ! docker compose -f "$BASE_FILE" -f "$TEMP_COMPOSE_FILE" config > "${PROJECT_ROOT}/docker/standard_project/docker-compose.devcontainer.yml"; then
