...
# standard_project Docker Cleanup Script
...
COMPOSE_PROJECT="standard_projectco"
DRY_RUN=false
FORCE=false
CLEAN_ALL=true
...
Docker cleanup script for standard_project project
...
# Print header
echo -e "${YELLOW}========================================${NC}"
echo -e "${YELLOW}  standard_project Docker Cleanup Tool   ${NC}"
echo -e "${YELLOW}========================================${NC}"
echo
...
if [ "$FORCE" = false ] && [ "$DRY_RUN" = false ]; then
    print_warning "This will remove Docker resources for the standard_project project"
    echo -e "${YELLOW}This includes:${NC}"
    echo "  - All containers with 'standard_project' or 'standard_projectco' in their name"
    echo "  - All VS Code devcontainer containers for this project"
    echo "  - Docker images: standard_project:*, project:latest (legacy)"
...
if [ "$CLEAN_IMAGES" = true ]; then
    ...
    # Remove standard_project images
    standard_project_images=$(docker images -q "${PROJECT_NAME}:*" 2>/dev/null || true)
    if [ -n "$standard_project_images" ]; then
        execute_command "docker rmi -f $standard_project_images 2>/dev/null" "Removing standard_project:* images"
    fi
    ...
    # Remove any images with standard_project in the name
