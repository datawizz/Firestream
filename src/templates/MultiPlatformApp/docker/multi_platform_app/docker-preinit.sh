...
set_env_variable() {
    local example_file_path="${PROJECT_ROOT}/.env.example"
    local expected_file_path="${PROJECT_ROOT}/.env"
...
# Define file paths
BASE_FILE="${PROJECT_ROOT}/docker/multi_platform_app/docker-compose.yml"
TEMP_COMPOSE_FILE="${PROJECT_ROOT}/docker/multi_platform_app/docker-compose.temp.yml"
...
if [ "$HOST_GPU_VENDOR" = "NVIDIA" ]; then
    OVERRIDE_FILE="${PROJECT_ROOT}/docker/multi_platform_app/docker-compose.gpu_nvidia.yml"
    ...
    if ! docker compose -f "$BASE_FILE" -f "$TEMP_COMPOSE_FILE" config > "${PROJECT_ROOT}/docker/multi_platform_app/docker-compose.devcontainer.yml"; then
