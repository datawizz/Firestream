# Superset chart options: `worker.*` (Celery worker Deployment).
#
# The worker section mirrors web's pod-spec shape (replicas, probes,
# resources, security context, affinity, autoscaling, networkPolicy,
# extraEnvVars, extraVolumes). Worker has NO `service` (it's a celery
# consumer, not a server) and NO `ingress`. Most knobs are
# `freeformType`-passthrough; we type the most-overridden leaves only.
{ lib, chartTypes, ... }:
...
in {
  options.superset.worker = mkOption {
    default = null;
    description = "Superset Celery worker deployment configuration";
