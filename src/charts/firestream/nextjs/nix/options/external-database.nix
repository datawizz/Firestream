# Nextjs chart options: `externalDatabase.*`.
#
# Active only when `postgresql.enabled = false`. The nextjs
...
#
# Nextjs's externalDatabase carries TWO password fields (the
...
# nextjs database when `externalDatabase.create = true`).
{ lib, ... }:

let
  inherit (lib) mkOption types;
in {
  options.nextjs.externalDatabase = mkOption {
    default = null;
    ...
    type = types.nullOr (types.submodule {
      ...
      options = {
        ...
        user = mkOption {
          type = types.nullOr types.str;
          default = null;
          description = "Non-root username for Nextjs";
        };

        password = mkOption {
          type = types.nullOr types.str;
          default = null;
          description = "Password for the non-root username for Nextjs";
        };

        database = mkOption {
          type = types.nullOr types.str;
          default = null;
          description = "Nextjs database name";
        };
