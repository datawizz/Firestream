# Odoo chart options: `postgresql.*` subchart passthrough.
#
# Bundled Bitnami postgresql (Chart.yaml v16.x.x). The odoo chart's
# values.yaml only documents `enabled`, `auth.username`, `auth.password`,
# `auth.database`, `auth.existingSecret`, `architecture` — those are
# the keys odoo's templates actually read. The full ~200-field
...
in {
  options.odoo.postgresql = mkOption {
    default = null;
    ...
    type = types.nullOr (types.submodule {
      ...
      options = {
        ...
        auth = mkOption {
          default = null;
          description = "PostgreSQL auth configuration consumed by Odoo";
          type = types.nullOr (types.submodule {
            freeformType = types.attrsOf types.anything;
            options = {
              username = mkOption {
