{
  description = "Firestream Superset 5.x - Pure Nix build replacing Bitnami stacksmith dependencies";
  ...
  outputs = { self, nixpkgs, flake-utils, firestream, pyproject-nix, uv2nix, pyproject-build-systems }:
    ...
    flake-utils.lib.eachSystem supportedSystems (system:
      let
        ...
        # Python version - Superset 5.x supports Python 3.10-3.12
        python = pkgs.python311;

        # Superset major version (actual version resolved from uv.lock)
        supersetVersion = "5";
      ...
      in {
        ...
        devShells.default = if isLinux then supersetModule.devShell else pkgs.mkShell {
          ...
          shellHook = ''
            echo "Superset 5.x development shell (macOS)"
