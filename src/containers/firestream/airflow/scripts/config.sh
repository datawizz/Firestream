...
if [[ "$AIRFLOW_COMPONENT_TYPE" == "webserver" || "$AIRFLOW_COMPONENT_TYPE" == "api-server" ]]; then
  ...
  if is_boolean_yes "${AIRFLOW_LDAP_ENABLE:-no}"; then
    ...
    python_conf_set "AUTH_LDAP_SERVER" "$AIRFLOW_LDAP_URI" "$AIRFLOW_WEBSERVER_CONF_FILE" "yes"
    python_conf_set "AUTH_LDAP_SEARCH" "$AIRFLOW_LDAP_SEARCH" "$AIRFLOW_WEBSERVER_CONF_FILE" "yes"
    python_conf_set "AUTH_LDAP_UID_FIELD" "$AIRFLOW_LDAP_UID_FIELD" "$AIRFLOW_WEBSERVER_CONF_FILE" "yes"
    python_conf_set "AUTH_LDAP_BIND_USER" "$AIRFLOW_LDAP_BIND_USER" "$AIRFLOW_WEBSERVER_CONF_FILE" "yes"
    python_conf_set "AUTH_LDAP_BIND_PASSWORD" "$AIRFLOW_LDAP_BIND_PASSWORD" "$AIRFLOW_WEBSERVER_CONF_FILE" "yes"
    python_conf_set "AUTH_USER_REGISTRATION" "$AIRFLOW_LDAP_USER_REGISTRATION" "$AIRFLOW_WEBSERVER_CONF_FILE" "no"
    [[ -n "$AIRFLOW_LDAP_USER_REGISTRATION_ROLE" ]] && \
      python_conf_set "AUTH_USER_REGISTRATION_ROLE" "$AIRFLOW_LDAP_USER_REGISTRATION_ROLE" "$AIRFLOW_WEBSERVER_CONF_FILE" "yes"
    [[ -n "$AIRFLOW_LDAP_ROLES_MAPPING" ]] && \
      python_conf_set "AUTH_ROLES_MAPPING" "$AIRFLOW_LDAP_ROLES_MAPPING" "$AIRFLOW_WEBSERVER_CONF_FILE" "no"
    python_conf_set "AUTH_ROLES_SYNC_AT_LOGIN" "$AIRFLOW_LDAP_ROLES_SYNC_AT_LOGIN" "$AIRFLOW_WEBSERVER_CONF_FILE" "no"
    python_conf_set "AUTH_LDAP_ALLOW_SELF_SIGNED" "$AIRFLOW_LDAP_ALLOW_SELF_SIGNED" "$AIRFLOW_WEBSERVER_CONF_FILE" "no"

    if [[ "$AIRFLOW_LDAP_USE_TLS" == "True" && -n "$AIRFLOW_LDAP_TLS_CA_CERTIFICATE" ]]; then
      python_conf_set "AUTH_LDAP_TLS_CACERTFILE" "$AIRFLOW_LDAP_TLS_CA_CERTIFICATE" "$AIRFLOW_WEBSERVER_CONF_FILE" "yes"
    fi
  fi

  # Configure hashing for Airflow 3
  if [[ $major_version -eq 3 ]]; then
    python_conf_set "FAB_PASSWORD_HASH_METHOD" "pbkdf2:sha256" "$AIRFLOW_WEBSERVER_CONF_FILE" "yes"
  fi
fi
