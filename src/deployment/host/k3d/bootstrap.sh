...
check_registry() {
  if k3d registry list | grep -q $PROJECT_NAME.$CONTAINER_REGISTRY_URL; then
    ...
    k3d registry create $PROJECT_NAME.$CONTAINER_REGISTRY_URL --port $CONTAINER_REGISTRY_PORT --cluster $PROJECT_NAME
  fi
}
...
create_cluster() {
  ...
  k3d cluster create $PROJECT_NAME \
    ...
    --registry-use k3d-$PROJECT_NAME.$CONTAINER_REGISTRY_URL:$CONTAINER_REGISTRY_PORT
...
configure_certificates() {
  ...
  bash /workspace/src/deployment/host/k3d/configure-tls-debian.sh
}
