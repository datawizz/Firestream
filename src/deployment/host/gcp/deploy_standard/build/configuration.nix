...
# Prodbox is a minimal NixOS host for Tailscale / Docker / Kubernetes
# This configuration is for a Google Compute Engine instance


{ config, pkgs, lib, serviceAccountFile ? "", hostname ? "prodbox", ... }:
let
  ...
  # Read credentials if files exist
  serviceAccountContent = safeReadFile serviceAccountFile;
in
{
  imports = [
