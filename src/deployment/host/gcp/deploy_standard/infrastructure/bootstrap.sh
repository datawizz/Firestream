...
pulumi config set --secret super_admin_key:cloudflare_api_token 'RbosNk9ZorfoWfOr81mQde0caVzsI0vx8qUjMXIy'
pulumi config set --secret super_admin_key:cloudflare_zone_id 'cab525ddbd3cce184bffe956495cd89d'
pulumi config set --secret super_admin_key:cloudflare_account_id 'eb7122e7cd9a9ef517a0e977dedda371'
...
pulumi config set --secret super_admin_key:tailscale_api_key 'tskey-api-kCF7zRk85711CNTRL-1UXynN81VMeKr7yGGA9hMeQ9uS8sRt6A8'
pulumi config set --secret super_admin_key:tailscale_tailnet_name 'unicorn-vernier.ts.net'
...
pulumi config set software_deployment:github_repo_1_url 'https://github.com/Cogent-Creation-Co/Firestream'
pulumi config set software_deployment:github_repo_1_branch 'main'
pulumi config set software_deployment:github_repo_1_commit '' # if empty, will use the latest commit

pulumi config set software_deployment:github_repo_2_url 'https://github.com/Pleasantrees/Pleasantrees'
pulumi config set software_deployment:github_repo_2_branch 'main'
pulumi config set software_deployment:github_repo_2_commit '' # if empty, will use the latest commit

pulumi config set software_deployment:github_repo_3_url 'https://github.com/Center-Point-Consulting-Group-LLC/pleasant_ui'
pulumi config set software_deployment:github_repo_3_branch 'main'
pulumi config set software_deployment:github_repo_3_commit '' # if empty, will use the latest commit

### New ###


# Base Identity Platform Configuration
pulumi config set identity_admin:plublic_domain 'centerpoint.consulting'
pulumi config set identity_admin:fully_qualified_domain_name 'auth.centerpoint.consulting'
pulumi config set --secret identity_admin:gcp_project_id 'centerpoint-identity-prod'
...
pulumi config set identity_admin:allowed_callback_domains '*.centerpoint.consulting'


### Tenant ###

# Tenant Identity Configuration
pulumi config set deploy_standard:tenant_id 'pleasantrees'
pulumi config set deploy_standard:customer_name 'pleasantrees'
pulumi config set deploy_standard:application_ 'pleasantrees'
pulumi config set deploy_standard:customer_email_domain 'enjoypleasantrees.com'
pulumi config set deploy_standard:tenant_callback '["enjoypleasantrees.com", "centerpoint.consulting"]'
pulumi config set deploy_standard:auth_callback_url 'https://pleasantrees.centerpoint.consulting/auth/callback'

# Domain Configuration
pulumi config set deploy_standard:domain_name 'centerpoint.consulting'
pulumi config set deploy_standard:fully_qualified_domain_name 'pleasantrees.centerpoint.consulting'
...
pulumi config set deploy_standard:gcp_project_id 'pleasantrees-dev'
