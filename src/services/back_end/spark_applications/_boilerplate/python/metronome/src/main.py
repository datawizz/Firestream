...
SPARK_MASTER = "k8://spark-master-svc.default.svc.cluster.local:7077"
SPARK_MASTER = "local[4]"
SPARK_APP_NAME = "SparkMetronome"
DEBUG = False
DEVICES = 5000
# Generate random device ids and broadcast this small piece of state
device_ids = json.dumps([uuid.uuid4().hex for x in range(DEVICES)])[:5]
...
def log_signals(df: DataFrame) -> None:
    ...
    if DEBUG:
        ...
        ).option("kafka.batch.size", "16384").option("kafka.linger.ms", "1").start().awaitTermination()

        #.option("kafka.batch.size", "16384").option("kafka.linger.ms", "1")
