...
pub enum Error {
    ...
    Database(#[from] firestream_api_server_db::Error),
...
impl IntoResponse for Error {
    fn into_response(self) -> axum::response::Response {
        match self {
            Error::Database(firestream_api_server_db::Error::NoRecordFound) => {
                StatusCode::NOT_FOUND.into_response()
            }
            Error::Database(firestream_api_server_db::Error::ValidationError(e)) => {
                validation_error(e).into_response()
            }
            Error::Database(firestream_api_server_db::Error::DbError(e)) => {
