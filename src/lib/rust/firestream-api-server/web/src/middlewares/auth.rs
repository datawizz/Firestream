...
use firestream_api_server_db::entities::users;
use tracing::Span;

/// Authenticates an incoming request based on an auth token.
///
/// This looks for a token in the `Authorization` header. If no token is present or no user exists with that token (see [`firestream_api_server_db::entities::users::load_with_token`]), a 401 response code is returned and the request is not processed further.
