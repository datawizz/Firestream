...
use firestream_api_server_config::{load_config, Config, Environment};
use firestream_api_server_db::{
...
///
/// A `DbTestContext` is passed as an argument to tests marked with the [`firestream_api_server_macros::db_test`] attribute macro. It is used to access the application under test as well as the database (which is the same database the application under test uses).
///
/// Example:
/// ```
/// #[db_test]
...
///
/// This function initializes a new instance of the application under test using the configuration for [`firestream_api_server_config::Environment::Test`]. The application is configured to use the same database that is also made available to the test itself via the test context. That database is a clone of the main test database that is only used by the particular test case to ensure isolation between test cases. It is automatically torn down after the test case completes (see [`teardown`]).
///
/// This function is not invoked directly but used inside of the [`firestream_api_server_macros::db_test`] attribute macro. The test context is automatically passed to test cases marked with that macro as an argument.
#[allow(unused)]
