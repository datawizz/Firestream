//! The firestream_api_server_web crate contains the application's web interface which mainly are controllers implementing HTTP endpoints. It also includes the application tests that are black-box tests, interfacing with the application like any other HTTP client.

use anyhow::Context;
use axum::serve;
use firestream_api_server_config::{get_env, load_config, Config};
...
///
/// 1. Determine the environment the application is running in (see [`firestream_api_server_config::get_env`])
/// 2. Load the configuration (see [`firestream_api_server_config::load_config`])
