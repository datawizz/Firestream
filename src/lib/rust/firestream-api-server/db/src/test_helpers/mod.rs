...
use firestream_api_server_config::DatabaseConfig;
...
/// This function is automatically called by the [`firestream-api-server-macros::db_test`] macro. The return connection pool is passed to the test case via the [`firestream-api-server-macros::DbTestContext`].
#[allow(unused)]
...
///
/// This function is automatically called by the [`firestream-api-server-macros::db_test`] macro. It ensures test-specific database are cleaned up after each test run so we don't end up with large numbers of unused databases.
