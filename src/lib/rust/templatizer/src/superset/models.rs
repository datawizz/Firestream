...
//!
//! This module defines the data structures for representing Superset
//! dashboards, datasets, charts, and related entities.

use serde::{Deserialize, Serialize};
use uuid::Uuid;

/// Root configuration structure for a Superset dashboard
...
impl DatasetBuilder {
    ...
    pub fn from_sql(name: &str, sql: &str) -> Self {
        let mut dataset = Dataset::default();
        dataset.name = name.to_string();
        dataset.table_name = name.to_string();
...
mod tests {
    ...
    fn test_dataset_builder() {
        let dataset = DatasetBuilder::new("test", "test_table")
        ...
        assert_eq!(dataset.table_name, "test_table");
        assert_eq!(dataset.columns.len(), 1);
    }

    #[test]
    fn test_virtual_dataset_builder() {
        let dataset = DatasetBuilder::from_sql("test_view", "SELECT * FROM users")
            .build();
    ...
    fn test_chart_builder() {
        ...
        assert!(chart.config.time.is_some());
    }
}
