"""Pulumi stack for the Firestream Odoo GKE example.
...
cluster_name = cfg.get("clusterName") or "firestream-odoo"
...
repo = gcp.artifactregistry.Repository(
    ...
    format="DOCKER",
    description="Firestream container images for the Odoo deployment",
    opts=pulumi.ResourceOptions(
...
# Kubernetes Secret keys the chart expects (odoo-password / password /
# postgres-password).
secret_specs = {
    "odoo-password": cfg.require_secret("odooPassword"),
    "odoo-db-password": cfg.require_secret("dbPassword"),
    "odoo-db-postgres-password": cfg.require_secret("dbPostgresPassword"),
}
...
deployer = gcp.serviceaccount.Account(
    "odoo-deployer",
    project=project,
    account_id="firestream-odoo-deployer",
    display_name="Firestream Odoo deployer (Cloud Build)",
)
