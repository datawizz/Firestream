"""Example DAG demonstrating Firestream's guest-DAG venv (config.airflow.dagWorkspace).

This example's headline customization: guest DAGs get their own Python
dependencies as a SEPARATE uv2nix venv, isolated from Airflow's own closure.

When `config.airflow.dagWorkspace` is set to a uv2nix workspace directory
(../dags-workspace: pyproject.toml + uv.lock), Firestream builds that workspace
into a separate venv baked at /opt/firestream/airflow/dags-venv (isolated from
Airflow's own venv at /opt/firestream/airflow/venv) and bakes two env vars into
every Airflow pod:
...
with DAG(
    ...
    # a single final line ("airflow-guest-dag-demo: ok") for exactly this reason;
    # for structured output, write a file or emit one final JSON line.
    run_guest_cli = BashOperator(
        task_id="run_guest_cli",
        bash_command='"$FIRESTREAM_DAGS_VENV/bin/airflow-guest-dag-demo"',
    )
