"""Pulumi stack for the Firestream Next.js GKE example.
...
Provisions, in one Google Cloud project, everything the deployment needs:
  ...
  * Secret Manager secrets holding the Postgres app + admin credentials
...
Manager and materialises the in-cluster Kubernetes Secret the Helm chart
...
cluster_name = cfg.get("clusterName") or "firestream-nextjs"
...
repo = gcp.artifactregistry.Repository(
    ...
    format="DOCKER",
    description="Firestream container images for the Next.js deployment",
    opts=pulumi.ResourceOptions(
...
# Kubernetes Secret keys the chart expects (password / postgres-password).
secret_specs = {
    "nextjs-db-password": cfg.require_secret("dbPassword"),
    "nextjs-db-postgres-password": cfg.require_secret("dbPostgresPassword"),
}
...
deployer = gcp.serviceaccount.Account(
    "nextjs-deployer",
    project=project,
    account_id="firestream-nextjs-deployer",
    display_name="Firestream Next.js deployer (Cloud Build)",
)
