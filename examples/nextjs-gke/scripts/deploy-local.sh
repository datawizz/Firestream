...
# images (incl. the custom nextjs image with the baked app), builds the
# customized chart bundle, authenticates kubectl to the GKE cluster, syncs the
# database Secret from Secret Manager, and deploys.
#
# Reads project-specific values from ../config.nix. Run from
# examples/nextjs-gke/ (or anywhere — it cd's to its own dir).
#
...
push_image nextjs-image firestream-nextjs
push_image postgresql-image firestream-postgresql

echo "==> Building customized chart bundle (nix build .#nextjs-chart)"
...
echo "==> Syncing Secret from Secret Manager"
